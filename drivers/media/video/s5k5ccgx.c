...
{
	u16 read_value = 0, capture_cnt = 0, capture_timeout = 500;
...
const char * const *s5k5ccgx_ctrl_get_menu(u32 id)
{
 ...
 *
 * */
static int s5k5ccgx_s_mbus_fmt(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *fmt)
{
	int err = 0;
	struct s5k5ccgx_state *state = to_state(sd);
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	...
	if(fmt->code == V4L2_MBUS_FMT_FIXED && fmt->colorspace != V4L2_COLORSPACE_JPEG){
...
static int s5k5ccgx_try_mbus_fmt(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *fmt)
{
	int num_entries;
	int i;
